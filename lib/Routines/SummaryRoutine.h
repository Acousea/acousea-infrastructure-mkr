...
public:
    ...
    Packet execute(const Packet& packet) override {
        ...
        SerialUSB.println("Sizeof Summary: " + String(sizeof(Summary)) + " bytes");
        if (payloadSize != sizeof(Summary)) {
            Serial.println("Invalid summary packet size");
            return NullPacket();
        }

        Summary summary;
        memcpy(&summary, payload, sizeof(Summary));
